...
***매크로 실행하는 법***
1) 명령모드에서 q[Name]을 입력하면 아래 기록중이라고 뜬다. name에 들어갈것은 맘대로 한글자 치면 된다. 예를 들어 o을 쳤다고 하자 그럼 qo로 시작한것이다.
2) 그럼 매크로가 동작중이다. 매크로를 실행시켰을 때 어떤 동작을 실행할지를 바로 이단계에서 만들어 놓아야 한다. 만드는 동안 실수를 했다면 :u로 작업 되돌리기를 한다.
3) 위에서 다 만들었다면 매크로가 끝났다는 의미로 q를 다시 입력하면 o라는 매크로가 완성된다.
4) 이제 등록된 매크로를 실행시키는 단계이다. 방법은 @[Name]이다. 여기서는 Name을 o로 설정했기에 @o하면 아까 했던 2번에서 만든 일련의 동작을 자동으로 실행해준다. 그러나 @[Name]은 매크로를 한번 실행해준다.
